...
#### Scenario: Updated cost does not affect existing products
- **WHEN** a product was created using raw material A with cost_per_unit = 10.00, then cost_per_unit is updated to 12.00
- **THEN** the product's cost calculation continues using 10.00; only new products use the updated cost
...
- **THEN** the result is exact without floating-point rounding (e.g., 10.6267 rounded to 10.63)

### Requirement: Cost per unit is used in product costing
The system SHALL provide cost_per_unit so that the recipe/product costing module can calculate ingredient costs for products.
